...
<html class="flex-layout">
	...
	<body style="background: #eef1f5; overflow: hidden;">
              ...
              <div class="indexTab" style="margin-bottom: 15px;">
				...
				<div class="unit overflowH records">
					...
					<ul class="rankList paddingB">
						<#if receivedContentRankList??>
							<#list receivedContentRankList as ranks >
								<li>
									...
									<span>获得回复${(ranks.receivedContentCount)!0}</span>
								</li>
							</#list>
						</#if>
					</ul>
		...
		<script src="${pictureVisitUrl}/htgc/js/receiveContentRanks.js"></script>
		<script>
			// 滚动到底部加载更多
			window.canLoad = true;
			...
			$(".body").on('scroll mousewheel', function(event) {
				if(window.scrollY <= maxScrollY && window.canLoad) {
					...
					setTimeout(function() {
						...
						$("#pageNo").val(pageNo);
							$.ajax({ 
								url: getBasePath()+'/frontpage/subjectRank/findReceivedContentRankListJson',
								type: "POST",
