...
<html class="flex-layout">
	...
	<body style="background: #eef1f5; overflow: hidden;">
		...
		<div class="modal-comfirm"  style="display:none;">
		  	<div class="content_zhongjiang" style="background:#54a5f6;height: 513px;">
			...
			success : function(data) {
				if (data!=null && data.status=='success') {
					if(data.showStatus=='1'){
						//alert("贡献您获得3乐豆奖励");
						$(".modal-comfirm").find("p").text("恭喜您获得奖励，请在我的获奖记录查看!");
						$(".modal-comfirm").css("display","block");
					}else{
						$(".modal-comfirm").find("p").text("发表评论成功!");
						$(".modal-comfirm").css("display","block");
						window.location.href=getBasePath()+"/frontpage/subjectTopicDetail/topicDetail?id="+topicId;
						return;
					}
				}
			},
