	...
	<body>
		<div style="margin: 15px;">
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				<legend>话题<#if topic??>修改<#else>添加</#if></legend>
			</fieldset>
			   ...
			   <#if topic??><input type="hidden" name="id" value="${(topic.id)!''}"></#if>
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" name="topicName" <#if topic??>value="${topic.topicName}" </#if>  lay-verify="required" autocomplete="off" placeholder="请输入话题名" class="layui-input" style="width:190px;">
					</div>
				</div>
				<div class="layui-form-item">
				...
				<#if topic??>
                    <div class="layui-form-item">
                        <label class="layui-form-label">是否支持投票</label>
                        <div class="layui-input-block">
                        <select name="supportVote" lay-verify="">
                          <option value="0" <#if topic.supportVote=='0'>selected</#if>>不支持</option>
                          <option value="1" <#if topic.supportVote=='1'>selected</#if>>支持</option>
                        </select>
                        </div>
                    </div>
                </#if>
				<div class="layui-form-item layui-form-text">
				    <label class="layui-form-label">话题详情</label>
				    <div class="layui-input-block">
			...
			layui.use(['form','jquery','upload','layedit','laydate'], function() {
				...
				debugger
				      layedit.sync(editIndex);
					   $.ajax({
				           url:'${request.contextPath}/backpage/subjectTopic/topic_save',
				           type:'post',
				           data:$('#form').serialize(),
				           success:function(data) { 
				             if("0000"==data.code){
				               layer.msg('添加成功',{time:2000})
				               setTimeout(function(){window.location="${request.contextPath}/backpage/subjectTopic/topic_list"},500);
				              }else{
