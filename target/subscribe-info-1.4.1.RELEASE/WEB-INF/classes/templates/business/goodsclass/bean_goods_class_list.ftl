...
<html>
	...
	<body>
		...
		<script>
			...
			layui.use(['icheck', 'laypage','layer'], function() {
				...
				laypage({
					...
					jump: function(obj, first) {
						...
						if(!first) {
						 $.ajax({
				           url:'${request.contextPath}/backpage/beanGoodsClass/getList',
				           type:'post',
				...
				$('#search').on('click', function() {
					...
					var name = $('#name').val();
					window.location.href = "${request.contextPath}/backpage/beanGoodsClass/list?status="+status+"&name="+name;
					
				});
				//跳转新增
				$('#toAdd').on('click', function() {
					window.location.href = "${request.contextPath}/backpage/beanGoodsClass/toinput"
				});
			...
			//跳转修改
			function toEdit(id){
				window.location.href = "${request.contextPath}/backpage/beanGoodsClass/toinput?id="+id;
			}
			//删除
			function delById(id){
				if(confirm("确定要删除？")){
					var url = '${request.contextPath}/backpage/beanGoodsClass/delete';
					$.post(url, {
		                'id':id
		            }, function(ret) {
					    if(ret == "0000"){
					    	layer.msg("删除成功");
					    	window.location.href = "${request.contextPath}/backpage/beanGoodsClass/list";
					    }else{
					    	layer.msg("删除失败");
					    }
		            });
