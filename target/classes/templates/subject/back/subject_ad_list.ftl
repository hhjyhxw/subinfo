...
<html>
	...
	<body>
		<div class="admin-main">
          <form id="form" action="${request.contextPath}/backpage/subjectAd/list" method="post">
			<blockquote class="layui-elem-quote">
		                   广告名称：<div class="layui-input-inline"><input type="text" name="adName" value="${(record.adName)!''}" id="adName" /></div>
				...
				广告位置： <div class="layui-input-inline"> 
					 <select name="posittionId" lay-verify="" id="posittionId">
			      		<option value="">--请选择--</option>
			      		<#if positionList??>
				      		<#list positionList as adPosition>
				      			<option value="${adPosition.id?c}"  <#if (record.posittionId?? && adPosition.id==record.posittionId)>selected="selected"</#if>>${adPosition.positionName}</option>
				      		</#list>
				      	</#if>
			   	 </select>
			   	  </div>
		       <a href="javascript:;" class="layui-btn layui-btn-small" id="search">
				 ...
				 <a href="${request.contextPath}/backpage/subjectAd/toinput" class="layui-btn layui-btn-small" id="search">
				...
				<div class="layui-field-box">
					<table class="site-table table-hover">
						...
						<#list page.list as record>
						    <tr>
								<td><img src="${request.contextPath}/${record.adImgurl!""}" height="70" width="255"/></td>
								<td>${(record.adName)!''}</td>
								<td>${(record.addJumpurl)!''}</td>
								<td>
									 <#if record.status=='0'>
                                     	  停用                                                        	
                                    </#if>
		...
		<script>
			...
			layui.use(['icheck', 'laypage','layer'], function() {
				...
				laypage({
					...
					jump: function(obj, first) {
						...
						if(!first) {
						 $.ajax({
				           url:'${request.contextPath}/backpage/subjectAd/getList',
				           type:'post',
				           data:{"pageNo":obj.curr,"adName":$('#adName').val(),"status":$('#status').val(),"posittionId":$('#posittionId').val()},
				           success:function(data) {
				            str = ''; 
                            $.each(data.list,function(i,val){
                                str += '<tr>'      
								str += '<th><img src="'+val.adImgurl+'"  height="70" width="70"/></th>';
								str += '<td>'+val.adName+'</td>'
								str += '<td>'+val.addJumpurl+'</td>'
			...
			//跳转修改
			function toEdit(id){
				window.location.href = "${request.contextPath}/backpage/subjectAd/toinput?id="+id;
			}
